...
export async function GET() {
  ...
  for (const c of challenges) {
    await sql`INSERT INTO challenges (title, description, difficulty, category, points)
      VALUES (${c.title}, ${c.desc}, ${c.d}, ${c.cat}, ${c.pts}) ON CONFLICT DO NOTHING`;
    inserted++;
  }
  return NextResponse.json({ ok: true, tablesCreated: true, challengesInserted: inserted });
}
